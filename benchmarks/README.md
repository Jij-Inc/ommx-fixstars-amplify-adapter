...
| `assignment` | Binary | 1次 | OneHot hints付き等式の変換 | `one-hot` | 10 / 20 / 30 |
...
| `tsp` | Binary | 2次 | OneHot hints付き等式の変換 | `one-hot` | 10 / 20 / 30 |
...
同一で、測定は `one-hot`(hints付き)だけを実施します。
v2の `one-hot` の結果は、v3の `regular` と `one-hot` 両方の比較相手を兼ねます。
モデル同一性は `tests/test_z_formulation_equivalence.py` で検証します。
